          ...
          >
            ...
            <div class="task-form">
              ...
              <div class="form-group" v-if="task.type === '句子模仿'">
                <label :for="'taskAudioFile-' + index">示例音频* (MP3格式)</label>
                <div class="file-upload">
                  ...
                  <div class="file-upload-ui">
                    <button @click="triggerTaskFileInput(index)" class="upload-btn">选择文件</button>
                    <span class="file-name">{{ task.audioFileName || '未选择文件' }}</span>
                  </div>
                </div>
  ...
  computed: {
    isFormValid() {
      ...
      return this.formData.tasks.every(task => {
        ...
        // 根据任务类型检查特定字段
        if (task.type === '句子模仿') {
          return task.content.trim() && task.audioFile;
  ...
  methods: {
    ...
    async saveSpeaking() {
      ...
      try {
        ...
        // 创建新口语训练
        await axios.post('http://localhost:5000/api/speaking/tasks', speakingData);
